...
\begin{document}
\title{Numerično reševanje Black-Schlesove enačbe}
\author{Matej Rojec}
\date{2025-01-29}
\maketitle
...
\begin{equation*}
    ...
    \end{array}\right.
    % začetek
    % dd{U}{t} = \frac{\sigma^2}{2}S^2 \frac{\partial^{2} {S}}{\partial {U}^{2}} + rS dd{U}{S} - rU  \textnormal{in } \mathcal{R}^T_V
    % U(S,0) = u_0(S)      ~~S \in [0,S^*] 
    % U(0,t) = u_a(t)      t \in [0,T] 
    % U(S^*,t) = u_b(t)    t \in [0,T]
    % konec
...
Obravnavamo primer evropske prodajne opcije s parametri $r = 0,08$, $\sigma = 0,4$, $T = 4$ in $K = 5$ in enačbo ??. 
...
Enačbo ?? z omenjenimi parametri lahko rešimo numerično z uporabo metode končnih diferenc. 
...
\begin{table}[h]
    ...
    \bottomrule
    \end{tabular}
    \caption{Napake pri uporabi eksplicitne metode za različne velikosti korakov $h_s$ in $h_t$ v času $t=1$.}
    \label{tab:napake}
\end{table}
    % Napis pod tabelo:
    % Napake, ki se pojavljajo pri uporabi eksplicitne metode za različne velikosti korakov $h_s$ in $h_t$ v času $t=1$.
% konec tabele
\bibliographystyle{plain}
\bibliography{viri (2)}
\begin{thebibliography}{9}
\bibitem{wiki}
Wikipedia.Black–Scholes model---{W}ikipedia{,} The Free Encyclopedia,2025,
